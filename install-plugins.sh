...
installFromFile() {
    if [[ -f $plugin ]]; then
        echo "Install from file $plugin"
        echo "Moving $plugin to $REF_DIR"

        mv $plugin $REF_DIR

        jpi_name="$(getFileName $plugin)"

        if [[ $plugin =~ .*\.hpi ]]; then
            cd $REF_DIR
            mv $jpi_name.hpi $jpi_name.jpi
            cd - > /dev/null 2>&1
        fi
        resolveDependencies $jpi_name

    else
        echo "$plugin is not a valid file"
        return 1
    fi
}
...
main() {
    ...
    for plugin in "$@"; do
        version=""
        if [[ $plugin =~ .*\.[hj]pi ]]; then
            # file
            installFromFile $plugin &
        else
            if [[ $plugin =~ .*:.* ]]; then
